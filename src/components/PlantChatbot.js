...
function welcomeMessage(plant) {
  if (
    ...
    return `Hi! Ako si **Plant Buddy** 🌿\nNaka-scan mo ang **${name}**. Magtanong ka tungkol sa alaga, tubig, liwanag, o sakit — sagot ako.`;
  }
  return "Hi! Ako si **Plant Buddy** 🌿\n**Hindi kailangan mag-scan** para magtanong — sabihin mo lang ang halaman o problema.\n\nHalimbawa: *Paano magtanim ng mangga?*, *Bakit dilaw ang dahon?*, *Alaga ng monstra*.\n\nKung may picture at gusto mong malaman ang pangalan, saka mag-**Scan Plant**.";
}
...
function PlantChatbot({ plant }) {
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      const metaLabel = (() => {
        ...
        if (data.offline) return data.note || "Local tips";
  ...
  const plantLabel =
    plant?.commonNameFilipino || plant?.commonName || null;
